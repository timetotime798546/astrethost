...
public class MainActivity extends Activity {
    ...
    private Button btnShapeCube;
    private Button btnShapePyramid;
    private Button btnShapeDiamond;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnShapeCube = (Button) findViewById(R.id.btnShapeCube);
        btnShapePyramid = (Button) findViewById(R.id.btnShapePyramid);
        btnShapeDiamond = (Button) findViewById(R.id.btnShapeDiamond);

        cardContainer = (LinearLayout) findViewById(R.id.cardContainer);
        cube3DView = (Cube3DView) findViewById(R.id.cube3DView);

        // Apply dynamic interactive 3D Tilt rotation effects on Card Container touch!
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        float normX = (event.getX() - (width / 2.0f)) / (width / 2.0f);
                        float normY = (event.getY() - (height / 2.0f)) / (height / 2.0f);

                        // Clamp values to keep tilt safe and gorgeous
                        normX = Math.max(-1.0f, Math.min(1.0f, normX));
                        normY = Math.max(-1.0f, Math.min(1.0f, normY));

                        v.setRotationY(normX * 7.5f);
                        v.setRotationX(-normY * 7.5f);
                        v.setTranslationX(normX * 5.0f);
                        v.setTranslationY(normY * 5.0f);
                        break;
                ...
                return false; // Return false to make sure children buttons remain responsive!
            }
        });
        ...
        btnVibeEnergetic.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                updateVibeUI();
                cube3DView.setVibe(currentVibe);
                SoundEngine.playEnergetic();
            }
        });
        ...
        btnVibeInspire.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                updateVibeUI();
                cube3DView.setVibe(currentVibe);
                SoundEngine.playInspire();
            }
        });

        // Shape Button Interactions
        btnShapeCube.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                cube3DView.setShape(Cube3DView.SHAPE_CUBE);
                updateShapeUI();
                SoundEngine.playClick();
                Toast.makeText(MainActivity.this, "Activated solid 3D Cube projection", Toast.LENGTH_SHORT).show();
            }
        });

        btnShapePyramid.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                cube3DView.setShape(Cube3DView.SHAPE_PYRAMID);
                updateShapeUI();
                SoundEngine.playClick();
                Toast.makeText(MainActivity.this, "Activated solid 3D Pyramid projection", Toast.LENGTH_SHORT).show();
            }
        });

        btnShapeDiamond.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                cube3DView.setShape(Cube3DView.SHAPE_DIAMOND);
                updateShapeUI();
                SoundEngine.playClick();
                Toast.makeText(MainActivity.this, "Activated solid 3D Octahedron projection", Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private void updateSoundToggleUI() {
        if (SoundEngine.isSoundEnabled()) {
            ...
            setActiveVibeStyle(btnSoundToggle, "#10B981", "#ECFDF5", "#A7F3D0"); // Rich Emerald Green
        } else {
            btnSoundToggle.setText("🔇 SOUND OFF");
            setActiveVibeStyle(btnSoundToggle, "#EF4444", "#FEF2F2", "#FCA5A5"); // Bold Rose Red
        }
    }
    ...
    private void updateShapeUI() {
        resetVibeButtonState(btnShapeCube);
        resetVibeButtonState(btnShapePyramid);
        resetVibeButtonState(btnShapeDiamond);

        int shape = cube3DView.getShape();
        if (shape == Cube3DView.SHAPE_CUBE) {
            setActiveVibeStyle(btnShapeCube, "#4F46E5", "#EEF2FF", "#C7D2FE");
        } else if (shape == Cube3DView.SHAPE_PYRAMID) {
            setActiveVibeStyle(btnShapePyramid, "#10B981", "#ECFDF5", "#A7F3D0");
        } else if (shape == Cube3DView.SHAPE_DIAMOND) {
            setActiveVibeStyle(btnShapeDiamond, "#F43F5E", "#FFF1F2", "#FECDD3");
        }
    }
    ...
    private void triggerInteraction() {
        ...
        // Play volumetric custom synth chord matching the current vibe
        SoundEngine.playCalculation(currentVibe);
    ...
    private void cycleWisdom() {
        ...
        SoundEngine.playClick(); // cyber-click feedback
