...
public class GameView extends View {
    ...
    private float terrainBaseHeight = 500f; // Dynamic base height scaled to view dimensions

    // Physics constants
    private static final float GRAVITY = 0.40f;
    ...
    public void initGame() {
        ...
        // Generate collectibles based on scalable terrain heights
    ...
    private float getTerrainHeight(float x) { 
        if (x < 0) return terrainBaseHeight;
    ...
    private float getTerrainControlHeight(int index) {
        if (index < 0) return terrainBaseHeight;
        if (index < 4) return terrainBaseHeight; // Flat starting area

        // Deterministic pseudorandom hill heights scaled with current screen size
        double bigHills = Math.sin(index * 0.12) * (terrainBaseHeight * 0.22f);
        double bumps = Math.cos(index * 0.75) * (terrainBaseHeight * 0.04f);

        // Steep slopes & ramps at specified track intervals
        double extraObstacle = 0;
        if (index > 15 && index < 20) {
            extraObstacle = -terrainBaseHeight * 0.15f; // ramp up
        } else if (index >= 35 && index < 42) {
            extraObstacle = -terrainBaseHeight * 0.25f; // high plateau
        } else if (index >= 65 && index < 72) {
            extraObstacle = terrainBaseHeight * 0.12f; // steep canyon
        } else if (index >= 110 && index < 120) {
            extraObstacle = -terrainBaseHeight * 0.30f; // big jump ramp
        }

        return (float) (terrainBaseHeight + bigHills + bumps + extraObstacle);
    }
    ...
    private void updatePhysics() {
        ...
        if (isBrakePressed) {
            if (grounded) {
                ...
                // Rotate left/backward in air
                angularVelocity -= 0.003f;
            }
        }

        // Balance tilt buttons
        ...
        // Restrict maximum speed limits to ensure absolute control and prevent clip bugs
        float maxSpeed = 15f;
        if (vx > maxSpeed) vx = maxSpeed;
        if (vx < -5f) vx = -5f;

        // Recompute positions for grounding adjustments
        ...
        // Ground collision checks with smooth elevation tracking (No jittering/bouncing)
        ...
        // Rider head collision check (triggers game over on crash/flip)
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        viewWidth = w;
        viewHeight = h;
        terrainBaseHeight = h * 0.75f; // Scale base line to bottom 75% of view port

        // Position preview bike on the slope under the initial main menu state
        if (!isPlaying && !isGameOver) {
            bikeX = 150f;
            bikeY = getTerrainHeight(bikeX) - 60f;
            cameraX = 0f;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 4. Draw Terrain with Camera translation matrix
        canvas.save();
        ...
        hillPath.moveTo(startX, viewHeight);
    ...
    private void drawBike(Canvas canvas) {
        ...
        helmetPaint.setColor(0xFF22D3EE); // Neon Helmet
