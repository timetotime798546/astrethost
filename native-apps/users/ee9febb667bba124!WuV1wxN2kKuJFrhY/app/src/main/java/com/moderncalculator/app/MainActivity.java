...
public class MainActivity extends Activity {
    ...
    // 3D Startup Loader views and variables
    private View layoutStartupLoader;
    private View loaderRing3DContainer;
    private View loaderInnerCore;
    private TextView tvLoaderPercent;
    private TextView tvLoaderStatus;
    private boolean isLoaderRunning = true;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Bind 3D Startup Loader Views
        layoutStartupLoader = findViewById(R.id.layoutStartupLoader);
        loaderRing3DContainer = findViewById(R.id.loaderRing3DContainer);
        loaderInnerCore = findViewById(R.id.loaderInnerCore);
        tvLoaderPercent = (TextView) findViewById(R.id.tvLoaderPercent);
        ...
        // Trigger Spectacular 3D Startup Loader immediately!
        layoutStartupLoader.setVisibility(View.VISIBLE);
    ...
    private void startStartupLoaderAnimation() {
        // Run audio sequence on a background thread to prevent UI freezing
        playStartupMelody();

        final long startTime = SystemClock.uptimeMillis();
        final long duration = 3000; // Duration of loader boots: 3.0 seconds
        ...
        Runnable loaderRunnable = new Runnable() {
            ...
            public void run() {
                ...
                tvLoaderPercent.setText(percent + "%");

                // Update diagnostic status texts based on timing progress
                if (percent < 20) {
                    tvLoaderStatus.setText("BOOTING SYSTEM CORES...");
                } else if (percent < 45) {
                    tvLoaderStatus.setText("CALIBRATING 3D ACCELEROMETERS...");
                } else if (percent < 70) {
                    tvLoaderStatus.setText("POLISHING SKEUOMORPHIC SHADOWS...");
                } else if (percent < 90) {
                    tvLoaderStatus.setText("TUNING low-latency chimes...");
                } else {
                    tvLoaderStatus.setText("READY!");
                }

                // Apply continuous complex 3D perspective rotations to loaders
                float rotAngle = (elapsed * 0.12f) % 360f;
                float density = getResources().getDisplayMetrics().density;

                // Outer loader spinner pitches dynamically
                ...
                loaderInnerCore.setRotationX(30f * (float) Math.cos(elapsed * 0.003));
                loaderInnerCore.setRotationY(-rotAngle * 1.5f);

                if (progress < 1f) {
                    loaderHandler.postDelayed(this, 16); // High frame rate rendering
                } else {
    ...
    private void playStartupMelody() {
        new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                    // Ascending digital melody frequency scale
                    int[] frequencies = { 329, 392, 494, 587, 659, 784, 987, 1175 };
                    int[] durations = { 80, 80, 80, 80, 100, 100, 120, 180 };
                    int[] intervals = { 90, 90, 90, 100, 110, 120, 140, 200 };

                    for (int i = 0; i < frequencies.length; i++) {
                        if (!isLoaderRunning) break;
                        playTone(frequencies[i], durations[i]);
                        Thread.sleep(intervals[i]);
                    }
                } catch (InterruptedException e) {
                    // Fail-safe thread exit
                }
            }
        }).start();
    }
    ...
    private void playTone(int frequency, int durationMs) {
        if (toneGenerator != null) {
            try {
                // Synthesize tone characteristics safely using standard ToneGenerator values
                int toneType = ToneGenerator.TONE_PROP_BEEP;
                if (frequency < 400) toneType = ToneGenerator.TONE_CDMA_PIP;
                else if (frequency < 700) toneType = ToneGenerator.TONE_CDMA_INTERCEPT;
                else toneType = ToneGenerator.TONE_CDMA_HIGH_L;
    ...
    private void dismissStartupLoader() {
        isLoaderRunning = false;

        // Trigger satisfying dual chime launch tone sequence
        new Thread(new Runnable() {
            @Override
            public void run() {
                playTone(1318, 120);
        ...
        // Graceful fade-out scaling dismiss animation
        layoutStartupLoader.animate()
                .alpha(0f)
                .scaleX(1.3f)
                .scaleY(1.3f)
                .setDuration(500)
                .withEndAction(new Runnable() {
                    @Override
                    public void run() {
                        layoutStartupLoader.setVisibility(View.GONE);
                    }
                });
    }
