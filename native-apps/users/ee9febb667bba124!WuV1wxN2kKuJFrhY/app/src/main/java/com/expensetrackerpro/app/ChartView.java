package com.expensetrackerpro.app;

import android.content.Context;
import android.graphics.Canvas;
import android.graphics.Color;
import android.graphics.Paint;
import android.graphics.RectF;
import android.util.AttributeSet;
import android.view.View;
import java.util.ArrayList;
import java.util.List;

public class ChartView extends View {
    public static class ChartData {
        public String label;
        public double value;
        public int color;

        public ChartData(String label, double value, int color) {
            this.label = label;
            this.value = value;
            this.color = color;
        }
    }

    private List<ChartData> dataList = new ArrayList<>();
    private Paint paint;
    private RectF rectF;

    public ChartView(Context context) {
        super(context);
        init();
    }

    public ChartView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    private void init() {
        paint = new Paint();
        paint.setAntiAlias(true);
        rectF = new RectF();
    }

    public void setData(List<ChartData> data) {
        this.dataList = data;
        invalidate(); // request layout redraw
    }

    @Override
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        if (dataList == null || dataList.isEmpty()) {
            paint.setColor(Color.parseColor("#64748B"));
            paint.setTextSize(36);
            paint.setTextAlign(Paint.Align.CENTER);
            paint.setShadowLayer(0, 0, 0, 0); // clear shadows for text
            canvas.drawText("No expenses registered yet", getWidth() / 2, getHeight() / 2, paint);
            return;
        }

        double total = 0;
        for (int i = 0; i < dataList.size(); i++) {
            total += dataList.get(i).value;
        }

        if (total == 0) return;

        int size = Math.min(getWidth(), getHeight()) - 180;
        if (size <= 0) return;

        int left = (getWidth() - size) / 2;
        int top = 40;
        rectF.set(left, top, left + size, top + size);

        // Configure a sleek elevated 3D depth layer to drawn elements
        paint.setShadowLayer(12, 0, 8, Color.parseColor("#3D0F172A"));

        float startAngle = 0;
        for (int i = 0; i < dataList.size(); i++) {
            ChartData data = dataList.get(i);
            float sweepAngle = (float) ((data.value / total) * 360.0);
            
            // Draw Pie Segment
            paint.setStyle(Paint.Style.FILL);
            paint.setColor(data.color);
            canvas.drawArc(rectF, startAngle, sweepAngle, true, paint);

            // Draw Clean Divider borders
            paint.setStyle(Paint.Style.STROKE);
            paint.setColor(Color.WHITE);
            paint.setStrokeWidth(4);
            canvas.drawArc(rectF, startAngle, sweepAngle, true, paint);

            startAngle += sweepAngle;
        }

        // Disable shadow layer to draw internal donuts overlay and texts cleanly
        paint.setShadowLayer(0, 0, 0, 0);

        // Draw inner white circular disk representing a premium modern donut plot layout
        paint.setStyle(Paint.Style.FILL);
        paint.setColor(Color.WHITE);
        canvas.drawCircle(getWidth() / 2, top + size / 2, size / 3, paint);

        // Dynamic legend layout labels drawn below the graphic plot
        paint.setTextSize(24);
        paint.setStrokeWidth(0);

        float legendY = top + size + 45;
        float legendX = 40;
        for (int i = 0; i < dataList.size(); i++) {
            ChartData data = dataList.get(i);
            
            // Solid bullet
            paint.setColor(data.color);
            canvas.drawCircle(legendX + 12, legendY - 8, 10, paint);

            // Print Label Metrics
            paint.setColor(Color.parseColor("#334155"));
            paint.setTextAlign(Paint.Align.LEFT);
            String labelDetail = data.label + " (" + String.format("%.1f%%", (data.value / total) * 100) + ")";
            canvas.drawText(labelDetail, legendX + 28, legendY, paint);

            legendX += 250;
            if (legendX > getWidth() - 220) {
                legendX = 40;
                legendY += 38;
            }
        }
    }
}