    ...
    private void loadData() {
        ...
        if (transactions.isEmpty()) {
            ...
            for (int i = 0; i < transactions.size(); i++) {
                ...
                TextView tvAvatar = (TextView) itemView.findViewById(R.id.tv_item_avatar);
                ...
                // Set dynamic circular avatar background color and single character letter
                int color = getCategoryColor(t.getCategory());
                GradientDrawable avatarBg = (GradientDrawable) tvAvatar.getBackground();
                if (avatarBg != null) {
                    avatarBg = (GradientDrawable) avatarBg.mutate();
                    avatarBg.setColor(color);
                    tvAvatar.setBackground(avatarBg);
                }
                String categoryStr = t.getCategory();
                String firstChar = (categoryStr != null && categoryStr.length() > 0) ? categoryStr.substring(0, 1).toUpperCase(Locale.US) : "T";
