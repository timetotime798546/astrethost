                    ...
                    public void run() {
                        try {
                            ...
                            if (code == 200) {
                                ...
                                final String result = jsonResponse.optString("result", "");
                                final String errorMsg = jsonResponse.optString("error", "");

                                runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        if ("completed".equalsIgnoreCase(status) || "success".equalsIgnoreCase(status)) {
                                            botTextView.setText(result.isEmpty() ? "Processing completed empty-handed." : result);
                                            enableInputs();
                                        } else if ("failed".equalsIgnoreCase(status) || "error".equalsIgnoreCase(status) || !errorMsg.isEmpty()) {
                                            botTextView.setText("Error: " + (errorMsg.isEmpty() ? "Processing job failed on server" : errorMsg));
                                            enableInputs();
                                        } else {
