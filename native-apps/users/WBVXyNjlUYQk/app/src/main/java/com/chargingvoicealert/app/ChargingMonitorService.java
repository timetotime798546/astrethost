...
public class ChargingMonitorService extends Service {
    ...
    public void onCreate() {
        ...
        // Start Foreground Notification safely with explicit type declarations required for SDK 34+
        try {
            if (Build.VERSION.SDK_INT >= 34) {
                // FOREGROUND_SERVICE_TYPE_SPECIAL_USE = 1073741824 (0x40000000)
                startForeground(NOTIFICATION_ID, buildMonitorNotification(), 1073741824);
            } else {
    ...
    private void speakConnected() {
        ...
        if (prefs.getBoolean("pref_alert_connected_enabled", true)) {
            String owner = prefs.getString("pref_owner_name", "Sir").trim();
            String customMsg = prefs.getString("pref_msg_connected", "apka mobile charge hone laga h").trim();
            String combined = owner.isEmpty() ? customMsg : (owner + " " + customMsg);
            speak(combined);
        }
    }
    ...
    private void speakDisconnected() {
        ...
        if (prefs.getBoolean("pref_alert_disconnected_enabled", true)) {
            String owner = prefs.getString("pref_owner_name", "Sir").trim();
            String customMsg = prefs.getString("pref_msg_disconnected", "charging remove kar diya hai").trim();
            String combined = owner.isEmpty() ? customMsg : (owner + " " + customMsg);
            speak(combined);
        }
    }
    ...
    private void speakPercentIncrease(int currentPercent) {
        ...
        if (pEnabled && iEnabled) {
            // Note: Owner Name is specifically excluded during percentage changes per the CRITICAL RULE.
            String template = prefs.getString("pref_msg_percent", "Battery badh gayi hai. Ab total battery %d percent hai");
            String replaced = template.replace("%d", String.valueOf(currentPercent));
            speak(replaced);
        }
    }
    ...
    private void speakFullyCharged() {
        ...
        if (prefs.getBoolean("pref_alert_full_enabled", true)) {
            String owner = prefs.getString("pref_owner_name", "Sir").trim();
            String customMsg = prefs.getString("pref_msg_full", "battery poori charge ho gayi hai").trim();
            String combined = owner.isEmpty() ? customMsg : (owner + " " + customMsg);
            speak(combined);
        }
    }
