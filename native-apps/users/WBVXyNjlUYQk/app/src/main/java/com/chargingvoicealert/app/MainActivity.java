...
public class MainActivity extends Activity {
    ...
    private EditText etMsgConnected;
    private EditText etMsgDisconnected;
    private EditText etMsgFull;
    private EditText etMsgPercent;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnSaveCustom = (Button) findViewById(R.id.btn_save_custom);

        etOwnerName = (EditText) findViewById(R.id.et_owner_name);
        etMsgConnected = (EditText) findViewById(R.id.et_msg_connected);
        etMsgDisconnected = (EditText) findViewById(R.id.et_msg_disconnected);
        etMsgFull = (EditText) findViewById(R.id.et_msg_full);
        etMsgPercent = (EditText) findViewById(R.id.et_msg_percent);
        ...
        // Load custom input preferences
        if (etOwnerName != null) etOwnerName.setText(prefs.getString("pref_owner_name", "Sir"));
        if (etMsgConnected != null) etMsgConnected.setText(prefs.getString("pref_msg_connected", "apka mobile charge hone laga h"));
        if (etMsgDisconnected != null) etMsgDisconnected.setText(prefs.getString("pref_msg_disconnected", "charging remove kar diya hai"));
        if (etMsgFull != null) etMsgFull.setText(prefs.getString("pref_msg_full", "battery poori charge ho gayi hai"));
        if (etMsgPercent != null) etMsgPercent.setText(prefs.getString("pref_msg_percent", "Battery badh gayi hai. Ab total battery %d percent hai"));

        // Initialize Preference Switch Values safely
        if (switchVoiceAlerts != null) switchVoiceAlerts.setChecked(prefs.getBoolean("pref_voice_alerts_enabled", true));
        if (switchPercentAlerts != null) switchPercentAlerts.setChecked(prefs.getBoolean("pref_percent_alerts_enabled", true));
        if (switchAlertConnected != null) switchAlertConnected.setChecked(prefs.getBoolean("pref_alert_connected_enabled", true));
        if (switchAlertDisconnected != null) switchAlertDisconnected.setChecked(prefs.getBoolean("pref_alert_disconnected_enabled", true));
        if (switchAlertPercent != null) switchAlertPercent.setChecked(prefs.getBoolean("pref_alert_percent_enabled", true));
        if (switchAlertFull != null) switchAlertFull.setChecked(prefs.getBoolean("pref_alert_full_enabled", true));
        ...
        if (switchVoiceAlerts != null) switchVoiceAlerts.setOnCheckedChangeListener(changeListener);
        if (switchPercentAlerts != null) switchPercentAlerts.setOnCheckedChangeListener(changeListener);
        if (switchAlertConnected != null) switchAlertConnected.setOnCheckedChangeListener(changeListener);
        if (switchAlertDisconnected != null) switchAlertDisconnected.setOnCheckedChangeListener(changeListener);
        if (switchAlertPercent != null) switchAlertPercent.setOnCheckedChangeListener(changeListener);
        if (switchAlertFull != null) switchAlertFull.setOnCheckedChangeListener(changeListener);

        // Save custom settings
        if (btnSaveCustom != null) {
            btnSaveCustom.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    String ownerName = etOwnerName.getText().toString().trim();
                    String msgConnected = etMsgConnected.getText().toString().trim();
                    String msgDisconnected = etMsgDisconnected.getText().toString().trim();
                    String msgFull = etMsgFull.getText().toString().trim();
                    String msgPercent = etMsgPercent.getText().toString().trim();

                    SharedPreferences.Editor editor = prefs.edit();
                    editor.putString("pref_owner_name", ownerName);
                    editor.putString("pref_msg_connected", msgConnected);
                    editor.putString("pref_msg_disconnected", msgDisconnected);
                    editor.putString("pref_msg_full", msgFull);
                    editor.putString("pref_msg_percent", msgPercent);
                    editor.apply();

                    Toast.makeText(MainActivity.this, "Customized Alert Settings Saved!", Toast.LENGTH_SHORT).show();
                }
            });
        }
    ...
    protected void onResume() {
        ...
        // Register receivers safely for dynamic custom local actions (mandatory on Android 14)
