...
public class MainActivity extends Activity implements GameView.GameListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Apply customized retro design parameters programmatically
        mOverlayStart.setBackground(createRetroCardDrawable(0xEE0A0A0F, 0xFF00FFCC, 24f));
        mOverlayGameOver.setBackground(createRetroCardDrawable(0xEE12080E, 0xFFFF3366, 24f));
        findViewById(R.id.hud_panel).setBackground(createRetroCardDrawable(0xEE0C0C14, 0xFF2F2F3E, 0f));

        mBtnPlay.setBackground(createStateButtonDrawable(0x1500FFCC, 0x4000FFCC, 0xFF00FFCC, 16f));
        mBtnPlay.setTextColor(0xFF00FFCC);

        mBtnToggleSound.setBackground(createStateButtonDrawable(0x15FFFFFF, 0x40FFFFFF, 0xFFFFFFFF, 16f));
        mBtnToggleSound.setTextColor(0xFFFFFFFF);

        mBtnRetry.setBackground(createStateButtonDrawable(0x15FF3366, 0x40FF3366, 0xFFFF3366, 16f));
        mBtnRetry.setTextColor(0xFFFF3366);
    ...
    private Drawable createRetroCardDrawable(int bgColor, int borderColor, float cornerRadius) {
        GradientDrawable gd = new GradientDrawable();
        gd.setColor(bgColor);
        gd.setCornerRadius(cornerRadius);
        gd.setStroke(4, borderColor);
        return gd;
    }

    private Drawable createStateButtonDrawable(int normalBg, int pressedBg, int borderColor, float cornerRadius) {
        GradientDrawable normalGd = new GradientDrawable();
        normalGd.setColor(normalBg);
        normalGd.setCornerRadius(cornerRadius);
        normalGd.setStroke(4, borderColor);

        GradientDrawable pressedGd = new GradientDrawable();
        pressedGd.setColor(pressedBg);
        pressedGd.setCornerRadius(cornerRadius);
        pressedGd.setStroke(4, borderColor);

        StateListDrawable sld = new StateListDrawable();
        sld.addState(new int[]{android.R.attr.state_pressed}, pressedGd);
        sld.addState(new int[]{}, normalGd);
        return sld;
    }
