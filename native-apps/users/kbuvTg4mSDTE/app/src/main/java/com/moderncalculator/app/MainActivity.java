...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.button_clear).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                onDeleteClick(); // Changed from onClearClick to onDeleteClick
            }
        });
    ...
    private void onDeleteClick() { // Renamed from onClearClick
        if (!currentInput.isEmpty()) {
            // Delete the last character from current input
            currentInput = currentInput.substring(0, currentInput.length() - 1);
            if (currentInput.isEmpty()) {
                // If input became empty after deleting the last character,
                // and we are starting a new calculation (no operator or previous result pending)
                // then display should show "0" to avoid a blank.
                if (newCalculation && currentOperator.isEmpty()) {
                    currentInput = "0";
                }
            }
        } else {
            // If currentInput is already empty (meaning result is displayed or initial state).
            // This acts as a "Clear Entry" or "Clear All Except History"
            result = 0;
            currentOperator = "";
            newCalculation = true;
            operatorPressed = false;
            currentInput = "0"; // Ensure '0' is displayed, ready for new input
        }
        updateDisplay();
    }
