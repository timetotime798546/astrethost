...
public class VoiceAccessibilityService extends AccessibilityService implements RecognitionListener {
    ...
    private static final long SESSION_TIMEOUT_MS = 600000; // Exactly 10 minutes (600 seconds)
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        ...
        if (eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED || 
            ...
            if (packageName != null) {
                Log.d(TAG, "Dynamic Window updated: " + packageName.toString());
            }
        }
    }
    ...
    // Modern Native Pulsing Animations matching AI Orb requirements
    ...
    public void onError(int error) {
        if (isSessionActive) {
            ...
            if (elapsed < SESSION_TIMEOUT_MS) {
                // Keep active listening continuous without interruptions
                startSpeechRecognizerListening();
            } else {
                stopListeningSession();
                setOrbState("idle");
    ...
    public void onResults(Bundle results) {
        ...
        if (matches != null && !matches.isEmpty()) {
            ...
            // Set orb to processing immediately
    ...
    // Helper to identify and skip voice input/microphone button options to prevent loops
    ...
    private Command parseVoiceCommand(String rawCommand) {
        ...
        // 4. COMPLEX SEARCH COMMANDS
        // e.g., "Search YouTube for Android App Builder"
        ...
        // Hinglish/Alternative Searches (e.g. "YouTube par Punjabi song search")
    ...
    private boolean launchAppByName(String appName) {
        ...
        // Direct package routes for common engines
    ...
    // Click UI node with recursive recovery and attempt retry loops
    ...
    // Types text, then instantly triggers automatic search/submission pipeline
    ...
    private void submitAfterTyping(AccessibilityNodeInfo editNode, String typedText) {
        AccessibilityNodeInfo root = getRootInActiveWindow();
        ...
        // Fallback: Perform standard IME Search/Enter action
        if (editNode != null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                submitted = editNode.performAction(AccessibilityNodeInfo.AccessibilityAction.ACTION_IME_ENTER.getId());
            } else {
                // Pre-Android 11 alternative: Send ACTION_CLICK on parents or submit candidate mappings
                submitted = editNode.performAction(AccessibilityNodeInfo.ACTION_FOCUS);
            }
            editNode.recycle();
        }
    ...
    private void verifyUIChangedAfterDelay() {
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                Log.d(TAG, "Verifying UI transition matches expectations...");
                AccessibilityNodeInfo root = getRootInActiveWindow();
                if (root != null) {
    ...
    private void performScroll(boolean scrollDown) {
        AccessibilityNodeInfo root = getRootInActiveWindow();
        if (root == null) {
            displayFloatingStatus("No scroll viewport");
            return;
        }
        ...
        if (!scrollableNodes.isEmpty()) {
            ...
            if (!scrolled) {
                displayFloatingStatus("Scroll action ignored");
            }
        } else {
            displayFloatingStatus("Scrollable view not found");
        }
    }
    ...
    // Submit / Enter Automation Pipeline
    ...
    private void findSubmitCandidatesRecursive(AccessibilityNodeInfo node, List<AccessibilityNodeInfo> results) {
        ...
        // Filter voice actions or microphones to protect execution flow
    ...
    private void executeSearchPipeline(final String appName, final String searchQuery) {
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                clickSearchIconWithRetry(searchQuery, 1, 5);
            }
        }, 2200);
    }

    private void clickSearchIconWithRetry(final String searchQuery, final int attempt, final int maxAttempts) {
        AccessibilityNodeInfo root = getRootInActiveWindow();
        if (root == null) {
            if (attempt < maxAttempts) {
                handler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        clickSearchIconWithRetry(searchQuery, attempt + 1, maxAttempts);
                    }
                }, 600);
            } else {
                displayFloatingStatus("Timeout loading " + searchQuery);
                setOrbState("error");
                returnOrbToReadyDelayed();
            }
            return;
        ...
        if (searchNode != null) {
            ...
            if (clicked) {
                ...
                retrySearchIcon(searchQuery, attempt, maxAttempts);
            }
        } else {
            retrySearchIcon(searchQuery, attempt, maxAttempts);
        }
    ...
    private void retrySearchIcon(final String searchQuery, final int attempt, final int maxAttempts) {
        if (attempt < maxAttempts) {
            handler.postDelayed(new Runnable() {
                @Override
                public void run() {
