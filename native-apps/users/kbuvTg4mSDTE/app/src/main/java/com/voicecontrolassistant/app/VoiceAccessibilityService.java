...
public class VoiceAccessibilityService extends AccessibilityService implements RecognitionListener {
    ...
    // Type text into active input text field
    private void performTypeWithRetry(final String text, final int attempt, final int maxAttempts) {
        ...
        if (editNode != null) {
            ...
            if (typed) {
                ...
                retryType(text, attempt, maxAttempts);
            }
        } else {
            retryType(text, attempt, maxAttempts);
        }
    ...
    private void retryType(final String text, final int attempt, final int maxAttempts) {
        if (attempt < maxAttempts) {
            handler.postDelayed(new Runnable() {
                @Override
                public void run() {
    ...
    private void findSubmitCandidatesRecursive(AccessibilityNodeInfo node, List<AccessibilityNodeInfo> results) {
        ...
        if (node.isClickable() && node.isEnabled()) {
            if (txt.equals("search") || txt.equals("go") || txt.equals("done") || txt.equals("submit")) {
                match = true;
            } else if (dsc.contains("search") || dsc.contains("submit") || dsc.contains("go")) {
                match = true;
            } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
                String viewId = node.getViewIdResourceName();
                if (viewId != null) {
                    String id = viewId.toLowerCase();
                    if (id.contains("search_button") || id.contains("search_btn") || id.contains("search_icon") || id.contains("submit") || id.contains("go_button")) {
                        match = true;
                    }
                }
            }
