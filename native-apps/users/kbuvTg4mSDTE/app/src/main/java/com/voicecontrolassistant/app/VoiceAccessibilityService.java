...
public class VoiceAccessibilityService extends AccessibilityService implements RecognitionListener {
    ...
    static class ParsedAction {
        String actionType; // OPEN, SEARCH, CLICK, TYPE, SCROLL_DOWN, SCROLL_UP, PLAY, BACK, HOME, SUBMIT, VOLUME_UP, VOLUME_DOWN, TORCH_ON, TORCH_OFF, RESTART_PHONE
        String target = "";
        String query = "";
        String rawText = "";
    ...
    private boolean containsActionVerb(String text) {
        ...
        String[] actionWords = {
                ...
                "scroll", "niche", "scroll down", "scroll up", "upar",
                "play", "chalao",
                "back", "piche", "home",
                "enter", "submit", "go",
                "volume", "torch", "flashlight", "restart", "reboot",
                "बढ़ाओ", "कम", "चालू", "बंद", "जलाओ", "बुझाओ", "रिस्टार्ट", "आवाज़"
        };
    ...
    private ParsedAction parseSingleActionSegment(String segment) {
        ...
        // 1. VOLUME UP COMMANDS (English, Hindi Devanagari & Hinglish representation)
        if (clean.contains("volume up") || clean.contains("increase volume") || clean.contains("volume badhao") || 
            clean.contains("sound badhao") || clean.contains("volume badhaen") || clean.contains("volume tez") ||
            clean.contains("आवाज बढ़ाएं") || clean.contains("आवाज़ बढ़ाओ") || clean.contains("आवाज बढाओ") || 
            clean.contains("आवाज तेज") || clean.contains("आवाज़ तेज़") || clean.contains("आवाज़ बढ़ाएं")) {
            act.actionType = "VOLUME_UP";
            return act;
        }

        // 2. VOLUME DOWN COMMANDS (English, Hindi Devanagari & Hinglish representation)
        if (clean.contains("volume down") || clean.contains("decrease volume") || clean.contains("volume kam karo") || 
            clean.contains("sound kam karo") || clean.contains("volume dheeme") || clean.contains("volume kam") ||
            clean.contains("आवाज कम करें") || clean.contains("आवाज़ कम करो") || clean.contains("आवाज कम करो") || 
            clean.contains("आवाज धीरे") || clean.contains("आवाज़ धीमी") || clean.contains("आवाज़ कम करें")) {
            act.actionType = "VOLUME_DOWN";
            return act;
        }

        // 3. TORCH ON COMMANDS (English, Hindi Devanagari & Hinglish representation)
        if (clean.contains("torch on") || clean.contains("turn on torch") || clean.contains("flashlight on") || 
            clean.contains("turn on flashlight") || clean.contains("torch jalao") || clean.contains("torch chalu") ||
            clean.contains("flashlight jalao") || clean.contains("flashlight chalu") || 
            clean.contains("टॉर्च चालू करो") || clean.contains("टॉर्च जलाओ") || clean.contains("टॉर्च ऑन") || 
            clean.contains("फ्लैशलाइट चालू") || clean.contains("फ्लैशलाइट ऑन")) {
            act.actionType = "TORCH_ON";
            return act;
        }

        // 4. TORCH OFF COMMANDS (English, Hindi Devanagari & Hinglish representation)
        if (clean.contains("torch off") || clean.contains("turn off torch") || clean.contains("flashlight off") || 
            clean.contains("turn off flashlight") || clean.contains("torch bujhao") || clean.contains("torch band") ||
            clean.contains("flashlight bujhao") || clean.contains("flashlight band") || 
            clean.contains("टॉर्च बंद करो") || clean.contains("टॉर्च बुझाओ") || clean.contains("टॉर्च ऑफ") || 
            clean.contains("फ्लैशलाइट बंद") || clean.contains("फ्लैशलाइट ऑफ")) {
            act.actionType = "TORCH_OFF";
            return act;
        }

        // 5. RESTART MOBILE COMMANDS (English, Hindi Devanagari & Hinglish representation)
        if (clean.contains("restart mobile") || clean.contains("restart phone") || clean.contains("reboot mobile") || 
            clean.contains("reboot phone") || clean.contains("phone restart") || clean.contains("mobile restart") ||
            clean.contains("रिस्टार्ट करो") || clean.contains("मोबाइल रिस्टार्ट") || clean.contains("फोन रिस्टार्ट") || 
            clean.contains("पुनारंभ करें") || clean.contains("पुनः आरंभ करें")) {
            act.actionType = "RESTART_PHONE";
            return act;
        }
    ...
    private void speakActionAnnouncement(ParsedAction action) {
        ...
        switch (action.actionType) {
            ...
            case "SUBMIT":
                ...
                break;
            case "VOLUME_UP":
                speech = "Increasing device volume.";
                break;
            case "VOLUME_DOWN":
                speech = "Decreasing device volume.";
                break;
            case "TORCH_ON":
    ...
    private void executeRestartAction(ActionCallback callback) {
        ...
        if (Build.VERSION.SDK_INT >= 28) { // Build.VERSION_CODES.P
            // Constant 6 represents GLOBAL_ACTION_POWER_DIALOG
            success = performGlobalAction(6);
        }
        if (success) {
            speakText("Opening power menu. Please select Restart.");
            callback.onSuccess();
        } else {
            // Try root reboot fallback
            try {
