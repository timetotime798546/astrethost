...
public class VoiceAccessibilityService extends AccessibilityService implements RecognitionListener {
    ...
    private int currentTotalCommandsCount = 0;
    private int currentCompletedCommandsCount = 0;
    private int currentFailedCommandsCount = 0;
    ...
    private synchronized void appendToDebugLog(String text) {
        java.io.File logFile = new java.io.File(getFilesDir(), "gemini_command_debug.log");
        java.io.FileWriter writer = null;
        try {
            writer = new java.io.FileWriter(logFile, true);
            writer.write(text + "\n");
        } catch (Exception e) {
            Log.e(TAG, "Error writing to debug log", e);
        } finally {
            if (writer != null) {
                try {
                    writer.close();
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    appendToDebugLog("========================================");
                    appendToDebugLog("REQUEST START");
                    appendToDebugLog("TIME: " + timeStr);
                    appendToDebugLog("");
                    appendToDebugLog("USER TEXT:");
                    appendToDebugLog(recognizedText);
                    appendToDebugLog("");

                    String response = makeGeminiCall(apiKey, finalModel, recognizedText);
                    
                    // Log the exact raw response received
                    appendToDebugLog("RAW GEMINI RESPONSE:");
                    appendToDebugLog(response);
                    appendToDebugLog("");

                    final String cleanedResponse = cleanGeminiResponse(response);

                    // Log the cleaned response
                    appendToDebugLog("CLEANED GEMINI RESPONSE:");
                    appendToDebugLog(cleanedResponse);
                    appendToDebugLog("");
                    ...
                    handler.post(new Runnable() {
                        ...
                        public void run() {
                            try {
                                ...
                                appendToDebugLog("PARSED COMMAND COUNT:\n" + commands.length());
                                appendToDebugLog("");

                                currentTotalCommandsCount = commands.length();
                                currentCompletedCommandsCount = 0;
                                currentFailedCommandsCount = 0;
                                ...
                                for (int i = 0; i < commands.length(); i++) {
                                    ...
                                    parsedLog.append("COMMAND ").append(i + 1).append(" PARSED:\n");
                                    parsedLog.append("action=").append(actionName).append("\n");

                                    if ("OPEN_APP".equals(actionName)) {
                                        parsedLog.append("app=").append(cmd.optString("app", "")).append("\n");
                                    } else if ("OPEN_CHAT".equals(actionName)) {
                                        parsedLog.append("contact=").append(cmd.optString("contact", "")).append("\n");
                                    } else if ("TYPE_MESSAGE".equals(actionName)) {
                                        parsedLog.append("text=").append(cmd.optString("text", "")).append("\n");
                                    } else if ("TYPE_TEXT".equals(actionName) || "SEARCH".equals(actionName)) {
                                        parsedLog.append("text=").append(cmd.optString("text", cmd.optString("query", ""))).append("\n");
                                    } else if ("CLICK".equals(actionName)) {
                                        parsedLog.append("target=").append(cmd.optString("target", "")).append("\n");
                                    } else if ("SCROLL".equals(actionName)) {
                                        parsedLog.append("direction=").append(cmd.optString("direction", "DOWN")).append("\n");
                                    } else if ("WAIT".equals(actionName)) {
                                        parsedLog.append("duration=").append(cmd.optInt("duration", 1000)).append("\n");
                                    }
                                    appendToDebugLog(parsedLog.toString());

                                    if (actionName.isEmpty()) {
                                        Log.e(TAG, "COMMAND FAILED: INVALID_COMMAND");
                                        appendToDebugLog("COMMAND " + (i + 1) + " VALIDATION:\nINVALID\n");
                                        appendToDebugLog("INVALID ACTION:\n" + actionName);
                                        appendToDebugLog("INVALID REASON:\nAction name is empty\n");
                                        continue;
                                    }
                                    ...
                                    if ("OPEN_APP".equals(actionName)) {
                                        ...
                                        if (parsed.target.isEmpty()) {
                                            isValid = false;
                                            invalidReason = "contact is empty";
                                        }
                                    } else if ("TYPE_MESSAGE".equals(actionName)) {
                                        ...
                                        if (parsed.query.isEmpty()) {
                                            isValid = false;
                                            invalidReason = "text/query is empty";
                                        }
                                        ...
                                        isValid = false;
                                        invalidReason = "Unsupported action: " + actionName;
                                    }
    ...
    private void handleParsingFailure(String errorMsg) {
        // Write complete request execution summary immediately
        appendToDebugLog("EXECUTION SUMMARY:");
        appendToDebugLog("Total commands: 0");
        appendToDebugLog("Completed: 0");
        appendToDebugLog("Failed: 0");
        appendToDebugLog("");
        appendToDebugLog("========================================");
        appendToDebugLog("REQUEST END");
        appendToDebugLog("");
    ...
    private void executeNextActionSequence() {
        if (pendingActions == null || currentStepIndex >= pendingActions.size()) {
            ...
            // Log Summary
            appendToDebugLog("EXECUTION SUMMARY:");
            appendToDebugLog("Total commands: " + (pendingActions != null ? pendingActions.size() : 0));
            appendToDebugLog("Completed: " + currentCompletedCommandsCount);
            appendToDebugLog("Failed: " + currentFailedCommandsCount);
            appendToDebugLog("");
            appendToDebugLog("========================================");
            appendToDebugLog("REQUEST END");
            appendToDebugLog("");
        ...
        // Before the existing executor executes a command, log the details
        appendToDebugLog("EXECUTING COMMAND " + stepNum + ":");
        String paramVal = !action.target.isEmpty() ? action.target : action.query;
        appendToDebugLog(action.originalAction + (paramVal.isEmpty() ? "" : " " + paramVal));
        appendToDebugLog("");
            ...
            public void run() {
                ...
                executeActionWithRetry(action, 1, 10, new ActionCallback() {
                    ...
                    public void onSuccess() {
                        ...
                        appendToDebugLog("COMMAND " + stepNum + " COMPLETED");
                        appendToDebugLog("");
                        currentCompletedCommandsCount++;
                    ...
                    public void onFailure(String errorMsg) {
                        ...
                        appendToDebugLog("COMMAND " + stepNum + " FAILED: " + errorMsg);
                        appendToDebugLog("");
                        currentFailedCommandsCount++;

                        // The remainder of the queue is aborted on execution failure
                        int remaining = pendingActions.size() - currentStepIndex - 1;
                        currentFailedCommandsCount += remaining;

                        // Log Summary
                        appendToDebugLog("EXECUTION SUMMARY:");
                        appendToDebugLog("Total commands: " + pendingActions.size());
                        appendToDebugLog("Completed: " + currentCompletedCommandsCount);
                        appendToDebugLog("Failed: " + currentFailedCommandsCount);
                        appendToDebugLog("");
                        appendToDebugLog("========================================");
    ...
    private void executeActionInternal(ParsedAction action, ActionCallback callback) {
        switch (action.actionType) {
            ...
            case "CLICK":
                ...
                break;
            case "TYPE":
                // For Search and Type commands, log the actual values
                if ("SEARCH".equals(action.originalAction) || "TYPE_TEXT".equals(action.originalAction)) {
                    appendToDebugLog("EXECUTING SEARCH:");
                    appendToDebugLog("query=" + action.query);
                    appendToDebugLog("");
                } else {
                    appendToDebugLog("EXECUTING TYPE:");
                    appendToDebugLog("text=" + action.query);
                    appendToDebugLog("");
                }
            ...
            case "HOME":
                ...
                break;
            case "SUBMIT":
                // For Submit commands, log the actual values
                appendToDebugLog("EXECUTING SUBMIT:");
                appendToDebugLog("action=ENTER");
                appendToDebugLog("");
