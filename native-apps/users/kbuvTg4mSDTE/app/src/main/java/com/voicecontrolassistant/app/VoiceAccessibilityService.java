...
public class VoiceAccessibilityService extends AccessibilityService implements RecognitionListener {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    // Strictly Parse & Validate JSON structure according to user guidelines
                    ...
                    for (int i = 0; i < commands.length(); i++) {
                        Object commandObject = commands.get(i);
                        if (!(commandObject instanceof JSONObject)) {
                            throw new IllegalArgumentException("Malformed command inside array");
                        }
                        JSONObject cmd = (JSONObject) commandObject;
                        if (!cmd.has("action")) {
                            throw new IllegalArgumentException("Command missing action field");
                        }
                        ...
                        if ("OPEN_APP".equals(actionName)) {
                            ...
                            if (!cmd.has("app") || cmd.getString("app").trim().isEmpty()) {
                                throw new IllegalArgumentException("OPEN_APP missing required 'app' parameter");
                            }
                            parsed.target = cmd.getString("app");
                        } else if ("OPEN_CHAT".equals(actionName)) {
                            parsed.actionType = "CLICK";
                            if (!cmd.has("contact") || cmd.getString("contact").trim().isEmpty()) {
                                throw new IllegalArgumentException("OPEN_CHAT missing required 'contact' parameter");
                            }
                            parsed.target = cmd.getString("contact");
                        } else if ("TYPE_MESSAGE".equals(actionName)) {
                            parsed.actionType = "TYPE";
                            if (!cmd.has("text") || cmd.getString("text").trim().isEmpty()) {
                                throw new IllegalArgumentException("TYPE_MESSAGE missing required 'text' parameter");
                            }
                            parsed.query = cmd.getString("text");
                        } else if ("SEND_MESSAGE".equals(actionName)) {
                            parsed.actionType = "SUBMIT";
                        } else if ("CLICK".equals(actionName)) {
                            parsed.actionType = "CLICK";
                            if (!cmd.has("target") || cmd.getString("target").trim().isEmpty()) {
                                throw new IllegalArgumentException("CLICK missing required 'target' parameter");
                            }
                            parsed.target = cmd.getString("target");
                        } else if ("SCROLL".equals(actionName)) {
                            String dir = cmd.optString("direction", "DOWN").toUpperCase();
                            if (!"DOWN".equals(dir) && !"UP".equals(dir)) {
                            ...
                            parsed.target = String.valueOf(duration);
                        } else {
                            throw new IllegalArgumentException("Unsupported action name: " + actionName);
                        }
    ...
    private String cleanGeminiResponse(String input) {
        ...
        // Find and extract raw JSON securely from code fences or loose markdown blocks
    ...
    private String makeGeminiCall(String apiKey, String model, String userPrompt) throws Exception {
        ...
        String systemPrompt = "You are an Android command parser. Convert the user's natural-language request (English/Hindi/Hinglish) into commands supported by the existing Android command executor. " +
                "Return JSON only. Never execute actions yourself. Never invent action names. Use only the provided supported actions and their required parameters. Preserve message text exactly as intended by the user. If the request cannot be represented using the supported actions, return commands as an empty array and error UNSUPPORTED_COMMAND.\n" +
                "Supported commands:\n" +
                "- OPEN_APP: parameter 'app' (e.g. 'WhatsApp', 'YouTube')\n" +
                "- OPEN_CHAT: parameter 'contact' (e.g. 'Rahul')\n" +
                "- TYPE_MESSAGE: parameter 'text' (exact text message content)\n" +
                "- SEND_MESSAGE: no parameters\n" +
                "- CLICK: parameter 'target' (matching UI button / text label)\n" +
                "- SCROLL: parameter 'direction' ('DOWN' or 'UP')\n" +
                "- BACK: no parameters\n" +
                "- HOME: no parameters\n" +
                "- VOLUME_UP: no parameters\n" +
                "- VOLUME_DOWN: no parameters\n" +
                "- TORCH_ON: no parameters\n" +
                "- TORCH_OFF: no parameters\n" +
                "- RESTART_PHONE: no parameters\n" +
                "- WAIT: parameter 'duration' (milliseconds)\n" +
                "Output ONLY a raw JSON structure matching: {\"commands\": [{\"action\": \"...\"}]}.";
    ...
    private void executeNextActionSequence() {
        ...
        displayFloatingStatus("Action " + stepNum + "/" + total + ": " + action.actionType + " " + (!action.target.isEmpty() ? action.target : action.query));
        Log.d(TAG, "Executing sequence: [" + stepNum + "/" + total + "] Type: " + action.actionType);
