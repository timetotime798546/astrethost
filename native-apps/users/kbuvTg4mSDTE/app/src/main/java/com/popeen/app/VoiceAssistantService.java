...
public class VoiceAssistantService extends Service {
    ...
    private long wakeSessionEndTime = 0;
    ...
    private synchronized void transitionToState(VoiceState newState) {
        ...
        switch (newState) {
            case WAKE_WORD_LISTENING:
                ...
                sendUpdateBroadcast(newState.name(), "Continuous voice recognition started. Say 'Hey Popeen' to activate.");
            ...
            case WAKE_WORD_DETECTED:
                ...
                updateNotification(statusText);
                // Start the 10-minute active session window
                wakeSessionEndTime = System.currentTimeMillis() + SESSION_DURATION_MS;
                sendUpdateBroadcast(newState.name(), "Wake phrase matched! Active session started for 10 minutes.");
                speakWakeWordPrompt();
                break;
            ...
            case COMMAND_LISTENING:
                ...
                updateNotification(statusText);
                sendUpdateBroadcast(newState.name(), "Direct command mode active. (Session remaining: " + remainingSec + "s)");
        ...
        speechRecognizer.setRecognitionListener(new RecognitionListener() {
            ...
            public void onReadyForSpeech(Bundle params) {
                if (isSessionActive() && currentState == VoiceState.COMMAND_LISTENING) {
                    long sec = (wakeSessionEndTime - System.currentTimeMillis()) / 1000;
                    sendUpdateBroadcast(currentState.name(), "Session Active (" + sec + "s remaining). Say 'Time' directly...");
                } else {
    ...
    private void handleSpeechResults(Bundle results) {
        ...
        // If currently waiting for "Hey Popeen"
        if (currentState == VoiceState.WAKE_WORD_LISTENING) {
        ...
        // If within active 10-minute session OR in command prompt listening mode
        else if (currentState == VoiceState.COMMAND_LISTENING) {
            // First check if the user is saying wake-phrase again (redundant, but reset session)
            ...
            if (redundantWake) {
                wakeSessionEndTime = System.currentTimeMillis() + SESSION_DURATION_MS; // reset timer
                sendUpdateBroadcast(currentState.name(), "Wake phrase repeated. Resetting 10 minutes session timer!");
                speakWakeWordPrompt();
            } else {
    ...
    private void restartListeningOrReset() {
        if (currentState == VoiceState.WAKE_WORD_LISTENING) {
            ...
            if (isSessionActive()) {
                ...
                sendUpdateBroadcast("WAKE_WORD_LISTENING", "10-minute session expired. Returning to wake word mode.");
                transitionToState(VoiceState.WAKE_WORD_LISTENING);
            }
        }
    }
    ...
    private void handleSpeechError(int error) {
        ...
        if (error != SpeechRecognizer.ERROR_NO_MATCH && error != SpeechRecognizer.ERROR_SPEECH_TIMEOUT) {
            sendUpdateBroadcast(currentState.name(), "Speech Engine Log: " + errorMsg);
        }
        ...
        if (currentState == VoiceState.WAKE_WORD_LISTENING) {
            ...
            if (isSessionActive()) {
                // Instantly resume command listening in the background to fulfill '10 minute direct talk'
                startCommandListening();
            } else {
                sendUpdateBroadcast("WAKE_WORD_LISTENING", "Session expired due to inactivity. Say 'Hey Popeen' to wake.");
                transitionToState(VoiceState.WAKE_WORD_LISTENING);
            }
        }
    }
    ...
    /**
     * Determines whether user text should be treated as Hindi.
     * Evaluates Devanagari unicode segments and standard Roman Hindi / Hinglish tokens.
     */
    private boolean isHindiLanguage(String text) {
        if (text == null) return false;

        // Check for Devanagari characters (range \u0900 to \u097F)
        ...
        // Tokenized Roman Hindi analysis
        ...
        String[] hindiKeywords = {
            ...
            "gadi", "ghadi", "kam", "shukriya"
        };

        // Instant match for specific Hinglish commands
        ...
        for (String t : tokens) {
            String cleanToken = t.replaceAll("[^a-zA-Z]", "");
            for (String hk : hindiKeywords) {
                if (cleanToken.equals(hk)) {
                    matchedHindiCount++;
                    break;
                }
            }
    ...
    private void processUserCommand(String speech) {
        ...
        if (isTimeCommand(speech)) {
            sendUpdateBroadcast(currentState.name(), "Command identified: TIME (Language: " + (isHindi ? "Hindi" : "English") + ")");
            speakTimeResponse(isHindi);
        } else {
            sendUpdateBroadcast(currentState.name(), "Unrecognized command: \"" + speech + "\" (Language: " + (isHindi ? "Hindi" : "English") + ")");
                ...
                mainHandler.post(new Runnable() {
                    ...
                    public void run() {
                        if ("WAKE_PROMPT".equals(utteranceId)) {
                            ...
                            // After speaking the time/response, check if 10 min window is still active
                            if (isSessionActive()) {
                                transitionToState(VoiceState.COMMAND_LISTENING);
                            } else {
                                transitionToState(VoiceState.WAKE_WORD_LISTENING);
    ...
    private void speakWakeWordPrompt() {
        ...
        if ("hi".equals(lang)) {
            ...
            locale = new Locale("hi", "IN"); // Use Hindi TTS voice to read Hinglish naturally
        } else {
    ...
    private boolean isHindiResponse(String text) {
        ...
        return lower.contains("abhi") || lower.contains("baje") || lower.contains("baja") || lower.contains("maf") || lower.contains("maaf") || lower.contains("boliye") || lower.contains("hoon");
    }

    private void speakDirectly(String text, String utteranceId) {
        Locale targetLocale = isHindiResponse(text) ? new Locale("hi", "IN") : new Locale("en", "IN");
        speakDirectly(text, utteranceId, targetLocale);
    }
    ...
    private void speakDirectly(String text, String utteranceId, Locale locale) {
        ...
        // 5. Select correct locale prior to execution
        // 7. Verify textToSpeech.isLanguageAvailable(locale)
        // 9. Gracefully handle errors if selected locale voice is missing
        try {
            int availability = tts.isLanguageAvailable(locale);
            if (availability >= TextToSpeech.LANG_AVAILABLE) {
                tts.setLanguage(locale);
            } else {
                // System fallback to English (India)
                tts.setLanguage(new Locale("en", "IN"));
            }
        } catch (Exception e) {
            try {
