...
public class VoiceAssistantService extends Service {
            ...
            public void run() {
                if (currentState == VoiceState.WAKE_WORD_LISTENING) {
                    try {
                        ...
                        sendUpdateBroadcast(currentState.name(), "Continuous match start exception: " + e.getMessage());
                        destroySpeech();
    ...
    private void startCommandListening() {
        try {
            ...
            sendUpdateBroadcast(currentState.name(), "Speech failure: " + e.getMessage());
            transitionToState(VoiceState.WAKE_WORD_LISTENING);
        }
    }
