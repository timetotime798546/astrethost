...
public class VoiceAssistantService extends Service {
    ...
    public static long getSessionEndTime() {
        return currentState == VoiceState.STOPPED ? 0 : currentState == VoiceState.WAKE_WORD_LISTENING ? 0 : 0;
    }
    ...
    private void sendUpdateBroadcast(String state, String log) {
        ...
        intent.putExtra("session_end_time", wakeSessionEndTime);
    ...
    private synchronized void transitionToState(VoiceState newState) {
        ...
        switch (newState) {
            ...
            case COMMAND_LISTENING:
                ...
                scheduleListening(600); // Wait 600ms safe delay
                break;
    ...
    private synchronized void initSpeech() {
        ...
        speechRecognizer.setRecognitionListener(new RecognitionListener() {
            ...
            public void onError(int error) {
                isListening = false;
                if (ignoreNextError && error == SpeechRecognizer.ERROR_CLIENT) {
                    ignoreNextError = false;
                    return;
                }
                ignoreNextError = false;
        ...
        restartRunnable = new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    if (speechRecognizer != null) {
                        if (isListening) {
                            return; // Already active, avoid concurrent triggers
                        }
                        isListening = true;
    ...
    private void handleSpeechError(int error) {
        ...
        stopSpeechListening(); // Safely calls cancel() instead of destroy() to stop click sounds
