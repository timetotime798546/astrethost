...
public class ThreeDLoaderView extends View {
    ...
    private void init() {
        ...
        linePaint.setShadowLayer(15, 0, 0, 0xFF00FFCC);

        gridPaint = new Paint();
        gridPaint.setColor(0x2800FFCC);
        gridPaint.setStrokeWidth(2.5f);
        gridPaint.setStyle(Paint.Style.STROKE);
        gridPaint.setAntiAlias(true);

        particlePaint = new Paint();
        particlePaint.setColor(0x7700FFCC);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // Retro Futuristic Background Grid
        drawRetroGrid(canvas, w, h, elapsed);
        
        // Interactive Dust Particles moving through Z space
        drawParticles(canvas, w, h);
        
        // Dynamic Animations variables
        angleX += 0.026f;
        angleY += 0.038f;
        angleZ += 0.014f;
        
        // Pulse animation sync with sine wave
        pulseFactor = 1.0f + 0.16f * (float) Math.sin(elapsed * 0.0055);
        
        // Cool neon glitch burst at intervals
        boolean isGlitchActive = (elapsed % 1800 < 90);
        if (isGlitchActive) {
            linePaint.setColor(0xFFFF0088); // Hyper glitched magenta pink
            linePaint.setShadowLayer(25, 0, 0, 0xFFFF0088);
        } else {
            linePaint.setColor(0xFF00FFCC); // Default Neon Mint Cyan
            linePaint.setShadowLayer(15, 0, 0, 0xFF00FFCC);
        }
        ...
        float scale = Math.min(w, h) * 0.21f * pulseFactor;
        float distance = 3.5f;
        ...
        for (int i = 0; i < 8; i++) {
            ...
            // Visual Glitch distortion displacement
    ...
    private void drawParticles(Canvas canvas, int w, int h) {
        for (int i = 0; i < PARTICLE_COUNT; i++) {
            particleZ[i] -= 0.016f * particleSpeed[i];
            if (particleZ[i] <= 0.1f) {
                particleX[i] = (random.nextFloat() - 0.5f) * 450f;
                particleY[i] = (random.nextFloat() - 0.5f) * 450f;
                particleZ[i] = 3.6f;
            ...
            float size = (4.5f / particleZ[i]);
            
            if (px >= 0 && px <= w && py >= 0 && py <= h) {
                int opacity = (int) ((1.0f - (particleZ[i] / 3.6f)) * 170);
